...
class Elevation_Map():
    ...
    def build_reward_map(self, starting_point):
        ...
        calc_steps(fields=[starting_point], steps=self.steps)
...
def result(arr):
    ...
    aas = np.where(arr == "a")
    step_list = []
    for row, col in zip(aas[0], aas[1]):
        a_pos = (row, col)
        elev_map = Elevation_Map(arr=arr)
        try:
            elev_map.build_reward_map(starting_point=a_pos)
        except:
            E = elev_map.end
            res=elev_map.reward_map[E]
            step_list.append(res)
            continue

    print(sorted(set(step_list))[1])
    return res
