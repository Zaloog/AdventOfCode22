...
def build_map(moves, n_knots):
    knots = [[0,0] for knot in range(n_knots)] 
    ...
    for direction, steps in moves: 
        ...
        for step in range(steps):
            # new pos head
            knots[0] = move_head(pos_head=knots[0], dir=direction, step=1)
            for i in range(1, n_knots):
                # new pos tail
                knots[i] = move_tail(pos_head=knots[i-1], pos_tail=knots[i])
            tails_visited_pos.append([*knots[-1]])
...
def result(lines):
    ...
    map = build_map(movement, n_knots=10) 
    print(map)
